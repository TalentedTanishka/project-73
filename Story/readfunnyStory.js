...
export default class readmoralStory extends React.Component{
  ...
  componentDidMount=async()=>{
    ...
    const ref = await db.collection("funny-story").get();
    ref.docs.map(doc=>{
      this.setState({
  ...
  SearchFilterFunction=async()=>{
      ...
      {
      const ref = await db.collection("funny-story").startAfter(this.state. dataSource).limit(10).get()
  ...
  Search=async(text)=>{
    ...
    {
      const ref = await db.collection("funny-story").where('title','==',text).startAfter(this.state.dataSource).limit(10).get()
      ref.docs.map(doc =>{
        this.setState({
          allStories:[...this.state.allStories,doc.data()],
         dataSource:doc
