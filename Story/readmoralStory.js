...
export default class readmoralStory extends React.Component{
  ...
  componentDidMount=async()=>{
    ...
    const ref = await db.collection("moral-story").get();
    ref.docs.map(doc=>{
      this.setState({
  ...
  SearchFilterFunction=async()=>{
      ...
      {
      const ref = await db.collection("moral-story").startAfter(this.state. dataSource).limit(10).get()
...
/>
        ...
        {this.state.allStories.map((iteam,index)=>{
          return(
            <View style={{borderBottomWidth:10,marginTop:10,borderColor:"purple",marginBottom : 20}} key={index}>
...
<View style={{backgroundColor:"yellow",margin:20}}>
<Text style={{fontSize : 40, textDecorationLine: 'underline',marginLeft:10}}>
         {"Moral of the Story : " + iteam.moral}
       </Text>
</View>
